...
class TestRunner {
  ...
  async runTests() {
    ...
    // wait for ock_node0 to be up + lerna.ok file to exist in dist/e2net/node0/
  ...
  async getNodesInfo() {
    ...
    for (const nodeNumber of Array(10).keys()) {
      const commandNodeUp = `docker ps | grep node${nodeNumber}_ock | grep Up | wc -l`
      const { stdout: stdoutNodeUp, stderr: stderrNodeUp } = await exec(commandNodeUp)
      if (stdoutNodeUp[0] === '0') {
        return
      }
      const commandDockerInspectNode = `docker ps --format "{{.Names}}" | grep node${nodeNumber}_ock | xargs docker inspect`
  ...
  async launchNodes() {
    for (let nodeNumber in this.nodes) {
      ...
      const commandLaunch = `docker ps --format "{{.Names}}" | grep ${nodeInfos.name}_ock | xargs -I {} sh -c 'docker exec -d {} bash ock.sh'`
